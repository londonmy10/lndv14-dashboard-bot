...
// Fontu bir kez kaydetmek yeterlidir
...
module.exports = async member => {
    ...
    // Veritabanı verilerini alıyoruz
    ...
    // Kanal ayarlanmamışsa çıkış yap
    if (!memberChannelId) return;
    
    const kanal = guild.channels.cache.get(memberChannelId);
    if (!kanal) return;

    // 1. KONTROL: Kara Liste (Eğer yasaklıysa sadece uyarı atar)
    if (isBlacklisted) {
        return kanal.send("⚠️ **Yasaklı kullanıcı geldi. Lütfen DİKKATLİ olun!**");
    }

    // 2. KONTROL: Gold Üye (Senin kodundaki mantığa göre Gold üyeyse resim göndermiyor)
    if (isGoldMember) return;

    // 3. AŞAMAM: Görsel Hazırlama
    ...
    try {
        ...
        const attachment = new AttachmentBuilder(canvas.toBuffer(), { name: 'giris-vortex.png' });

        // Kanala gönder
        await kanal.send({ files: [attachment] });

    } catch (e) {
        console.error("Giriş görseli hazırlanırken hata oluştu:", e);
    }
}